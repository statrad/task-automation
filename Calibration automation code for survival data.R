...
GND.calib = function(pred, tvar, out, cens.t, groups, adm.cens){
  ...
  test <- c(df=numcat-1, chi2gw=sum(hltab$GND_component),pvalgw=1-pchisq(sum(hltab$GND_component),numcat-1))
  return(list(hltab = hltab[ ,c(1,2,3,4,10,5,6,9,7,11)], test = test))
} # GND.calib

calib_func <- function(coxmodel, train, valid, time, stat, adm.cens, xlab, ylab){
  
  # training
  survfit=survfit(coxmodel, newdata=train, se.fit=FALSE) # training set : self-validation
  survpr4K=survfit$surv[nrow(survfit$surv),]
  estsurv=survpr4K
  estinc=1-survpr4K
  dec=as.numeric(cut2(estinc, g=10))
  
  GND.result_train=GND.calib(pred=estinc, tvar=train[, time], out=train[, stat], cens.t=4000, groups=dec, adm.cens=adm.cens)
  
  # validation
  survfit=survfit(coxmodel, newdata=valid, se.fit=FALSE) # validation set : trained by training set and validated by validation set
  survpr4K=survfit$surv[nrow(survfit$surv),]
  estsurv=survpr4K
  estinc=1-survpr4K
  dec=as.numeric(cut2(estinc, g=10))
  
  GND.result_valid=GND.calib(pred=estinc, tvar=valid[, time], out=valid[, stat], cens.t=4000, groups=dec, adm.cens=adm.cens)
  
  # print
  par(mfrow=c(2, 1))
  list(plot(kmperc~expectedperc, GND.result_train$hltab, type="o",pch=19, xlim=c(0,1), ylim=c(0,1), main = 'training', 
            xlab = xlab, ylab = ylab), 
       abline(a=0, b=1, col = 'red', lty=2),
       plot(kmperc~expectedperc, GND.result_valid$hltab, type="o",pch=19, xlim=c(0,1), ylim=c(0,1), main = 'validation',
            xlab = xlab, ylab = ylab),
       abline(a=0, b=1, col = 'red', lty=2))
  return(list(GND.result_train, GND.result_valid))
}
...
d<-flchain # read the data

adm.cens=3500 # we decided we are interested in calibration at 4000 days, so censor after 4000
d$fu.time <- pmin(d$futime, adm.cens)
d$status <- ifelse(as.numeric(adm.cens < d$futime), 0, d$death)

train_ind <- sample(1:nrow(d), 0.5*nrow(d))
train <- d[train_ind, ]
valid <- d[-train_ind, ]

coxmodel <- coxph(data=train, Surv(fu.time, status) ~ age + sex + flc.grp + lambda) # cox model

png('calib.png')
calib_func(coxmodel=coxmodel, train=train, valid=valid, time='fu.time', stat='status', adm.cens = 3500, xlab = 'Predicted', ylab = 'Observed')
dev.off()
...
fit <- coxph(Surv(fu.time,status)~age+sex+flc.grp+lambda, data=train, x=T, y=T)
pec_plot <- calPlot(fit,time=3500,data=d)
pec_plot$summary
